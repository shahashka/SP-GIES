...
dataset_OI_path <- file.path(paste("./insilico_size10_3/data_combine.csv", sep=""), fsep=.Platform$file.sep)
...
dataset_O_path <- file.path(paste("./insilico_size10_3/data_obs.csv", sep=""), fsep=.Platform$file.sep)
...
for (network in ['ER', 'scale', 'small']) {
    for (x in 0:num_graphs-1) {
        ...
        dataset <- dataset[,1:ncol(dataset)-1]
        targets.index <- targets.index + 1
        sp_gies(dataset, targets, targets.index, save_path=paste("./random_test_set_",as.character(num_nodes),"_small/", sep=""))
        gies(dataset, targets, targets.index, save_path=paste("./random_test_set_",as.character(num_nodes),"_small/", sep=""))

        dataset_O_path <- file.path(paste("./random_test_set_",as.character(num_nodes), "_", network, "/data_obs_", x,".csv", sep=""), fsep=.Platform$file.sep)
        dataset <- read.table(dataset_O_path, sep=",", header=TRUE)
        targets.index <- dataset[,ncol(dataset)]
        targets <- as.matrix(unique(targets.index))
        targets <- targets[targets > 0]
        ...
        dataset <- dataset[,1:ncol(dataset)-1]
        targets.index <- targets.index + 1
        sp_gies(dataset, targets, targets.index, save_path=paste("./random_test_set_",as.character(num_nodes),"_small/obs_", sep=""), save_pc=TRUE)
        gies(dataset, targets, targets.index, save_path=paste("./random_test_set_",as.character(num_nodes),"_small/obs_", sep=""))
    }
}