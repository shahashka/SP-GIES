...
def get_scores(alg_names, networks, ground_truth, repeats):
...
def test_regulondb():
    ...
    aracne_network = pd.read_csv("./regulondb2/output/network.txt", sep='\t',header=0)
    ...
    get_scores(["ARACNE-AP", "CLR", "PC-O", "SP-GIES-OI", "GIES-OI", "GIES-O", "EMPTY"],
               [aracne_graph, clr_graph, pc_graph, sp_gies_graph, gies_graph ,
                gies_o_graph,np.zeros((len(genes), len(genes)))], true_graph)
